...
| 13 | Lead capture | **DONE** | Form live on the results page. Valid address → `?saved=1` + row in `leads`; `not-an-email` → `?error=email`. **Real SMTP delivery live since 2026-09-13** (Proton SMTP wired via `/opt/settleflow/.env`, EnvironmentFile drop-in): verified end-to-end — notify → `leads.delivered=1`, message accepted by smtp.protonmail.ch. The code never simulated a send, so delivered=1 is the proof. |
